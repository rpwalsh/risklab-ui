...
export function WorkbenchPanel({
  ...
  const panelState = panelId && workbench ? workbench.state.panels[panelId] : localState;
  const collapsed = panelState?.collapsed ?? false;

  const setPanelState = (patch: Partial<WorkbenchPanelState>) => {
    if (panelId && workbench) {
      workbench.actions.patchPanelState(panelId, patch);
      return;
    }
  ...
  return (
    <div {...rest} className={cx('rlwb-filter-bar', className)}>
      {filters.map((definition) => {
        ...
        const currentValue = scope === 'panel'
          ? definition.panelId
        ...
        return (
          <div
            key={`${scope}:${definition.panelId ?? 'global'}:${definition.key}`}
            className="rlwb-filter-group"
          >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (scope === 'panel' && definition.panelId) {
                        ...
                        actions.setFilter(definition.key, nextValue);
                      }
                    }}
                  >
                    {option.label}
...
export function TimeRangeControl({
  ...
  return (
    <div {...rest} className={cx('rlwb-time-range', className)}>
      ...
      <div className="rlwb-filter-group__options">
        {options.map((option) => {
          const active = areTimeWindowsEqual(state.timeWindow, option.value);
          return (
            <button
              key={String(option.label)}
              type="button"
...
export function EntityInspector({
  ...
  const workbench = useWorkbenchOptional();
  const selection = workbench?.state.selection ?? null;
